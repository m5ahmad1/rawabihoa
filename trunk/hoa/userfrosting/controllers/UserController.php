...
class UserController extends \UserFrosting\BaseController {
    ...
    public function formActivateUser($user_id){
        ...
        if ($render == "modal")
            $template = "components/common/user-Active-modal.twig";
        else
            $template = "components/common/user-Active-panel.twig";
        ...
        $user_units =Unit::findbyUser($user_id,$target_user->flag_verified);
      

        $neighborhood = Neighborhood::getAllNeighborhoods();
        $this->_app->render($template, [
            "box_id" => $get['box_id'],
            "box_title" => "Accept User",
            "submit_button" => "Activate User",
    ...
    public function formUserEdit($user_id){
        ...
        // Determine authorized fields
        $fields = ['display_name', 'email', 'title', 'locale', 'groups','phone_number','primary_group_id'];
        $show_fields = [];
        $disabled_fields = [];
        $hidden_fields = [];
        foreach ($fields as $field){
    ...
    public function updateUser($user_id){
        ...
        */
        ...
        if(isset($post['flag_verified']) && $post['flag_verified'] == "1"){
            $unint = Unit::updatUnitOwner($user_id);
        }
        ...
        foreach ($post as $name => $value) {
            if ($name == "groups" || ($target_user->attributeExists($name) && $post[$name] != $target_user->$name)){
             ...
             else if($name == "unit_change"){
               
                if($post["unit_change"] != ""){
                 
                    $user_units =Unit::updateUserUnits($user_id,$post["unit_change"],$target_user->flag_verified);
                }
             }
	...
	public function getUserId()
	{
		...
		//$session = $this->_app->ci->session;
		 ...
		 {
			echo $_SESSION["userfrosting"]["user_id"];
		 }	
		 else
		 {
			 echo null;
	...
	{
		$row=User::where('user_name',$user_name)->get();
		return $row[0]['id'];
	}
	...
	{
		$user=new User;
		return json_encode($user->getUsersList());
	}
     public function getUsersListt()
    {
        $user=new User;
        return json_encode($user->getUsersListt());
    }
}
