...
class CUnit extends UFModel
{
    ...
    public function updateFixedBill($post)
    {
        ...
        if ($bill == null) {
            ...
            $bill = Capsule::table($link_table_unit_bills)->where('year', $year)->update(['billAmount' => serialize($billAmount)]);
...
/*                 $returnData['due' . '_' . $value . '_' . $i] = $billSum;
    ...
    public function getUnitsPaymentsByDefualtNeighborhood()
    {
        $year = date('Y');
        $unitsPayments = Unit::leftJoin('uf_user', 'uf_unit.user_id', '=', 'uf_user.id')
        ->select('uf_unit.rawabicode', 'uf_unit.owner_name', 'uf_unit.neighborhood', 'uf_unit.id AS unit_id', 'uf_unit.user_id AS unit_owner')->where('uf_unit.neighborhood','2' )->get();
        $link_table = Database::getSchemaTable('bills')->name;
        $where = array('uf_payments.year' => $year ,'uf_bills.year' => $year);
        $payment      = Capsule::table($link_table)
            ->leftjoin('uf_payments', 'uf_bills.unit_id', '=', 'uf_payments.unit_id')
    ...
    {   
        $year = date('Y');
        $link_table_n = Database::getSchemaTable('neighborhood')->name;
        $where= array('uf_neighborhood.neighborhood_en' =>$neighborhood);
        $neighborhoodFromNeiT   = Capsule::table($link_table_n)
            ->select('uf_neighborhood.id')
             ->where($where)->get();
             $NeiID;
             for($j = 0; $j < count($neighborhoodFromNeiT); $j++) {
                $NeiID = $neighborhoodFromNeiT[$j]["id"];
             }
            
             $unitsPayments = Unit::leftJoin('uf_user', 'uf_unit.user_id', '=', 'uf_user.id')
             ->select('uf_unit.rawabicode', 'uf_unit.owner_name', 'uf_unit.neighborhood', 'uf_unit.id AS unit_id', 'uf_unit.user_id AS unit_owner')->where('uf_unit.neighborhood', $NeiID )->get();
             $link_table = Database::getSchemaTable('bills')->name;
             $where = array('uf_payments.year' => $year ,'uf_bills.year' => $year);
             $payment      = Capsule::table($link_table)
                 ->leftjoin('uf_payments', 'uf_bills.unit_id', '=', 'uf_payments.unit_id')
