...
function bindUserTableButtons(table) {
    ...
    $(table).find('.js-billfixed-create').click(function(){
    	userForm('dialog-billfixed-create');
    });
    ...
    $(table).find('.js-user-delete').click(function() {
     ...
     $(table).find('.js-user-accept').click(function() {
        var btn = $(this);
        var user_id = btn.data('id');
        var user_email = btn.data('email');
        acceptUserDialog('dialog-user-accept', user_id, user_email);
    });		 	        
}
...
function acceptUserDialog(box_id, user_id, name){
	// Delete any existing instance of the form with the same name
	if($('#' + box_id).length ) {
		$('#' + box_id).remove();
	}
	
    var url = site['uri']['public'] + "/forms/confirm/" + user_id;
    
	var data = {
		box_id: box_id,
		box_title: "Accept User",
		confirm_message: "Are you sure you want to accept the user " + name + "?",
		confirm_button: "Yes, send acctivation email"
	};
	...
	.done(function(result) {		
		...
		$('#' + box_id + ' .js-confirm').click(function(){
            ...
            var url = site['uri']['public'] + "/account/register1/";
		...
		$('#' + box_id + ' .js-regret').click(function(){
			var url = site['uri']['public'] + "/account/Regret";
			csrf_token = $("meta[name=csrf_token]").attr("content");
            var data = {
                email:name,
                csrf_token: csrf_token
...
function userForm(box_id, user_id) {	
	...
	.done(function(result) {
		...
		$(".edit-units").on('click',function(e){
			e.preventDefault();
			console.log($(this).attr("data-id"));
			var selector = $(this).closest("tr");

			selector.find(".text").addClass("hide");
			selector.find("select.neighborhood").removeClass("hide");
			if($(this).text() == "Save"){
			...
			$(selector).find(".neighborhood").change(function(){
                    ...
                    dataType: 'json'
                        ...
                        for(row in result){
                            selector.find(".rc").append($('<option>', { 
                                ...
                                text : result[row].rawabicode 
                            }));
                        }
                        selector.find(".rc").removeClass('hide');
                    });
				}
			});

			$(selector).find(".rc").change(function(){
				var selected = $(this).val();
				selector.find(".tc option[value!='0']").remove();
                    ...
                    dataType: 'json'
                        ...
                        for(row in result){
                            selector.find(".tc").append($('<option>', { 
                                value: result[row].id,
                                text : result[row].topocode 
                            }));
                        }

                        selector.find(".tc").removeClass('hide');
                    });
                }
			});

			$(selector).find(".tc").change(function(){
				var selected = $(this).val();
        ...
        ufFormSubmit(
            ...
            function(){
            	...
            	var units_id = "";
            	$(".modal-unit-table tbody tr").each(function(){
            		if($(this).find(".text").hasClass('hide') &&
            				$(this).find(".tc").val() != "0"){
        				units_id += $(this).find(".tc").val()+",";
        			}
            		else{
            			units_id += $(this).attr("data-id")+",";
            		}
            	});
				$("#unit_change").val(units_id);
