...
class AccountController extends \UserFrosting\BaseController
{
    ...
    public function pageRegister($can_register = false)
    {
        ...
        $unassignedNeighborhood = $neighborhood->getAllNeighborhoods();

        $this->_app->render('account/register.twig', [
            'captcha_image' => $this->generateCaptcha(),
            'validators' => $this->_app->jsValidator->rules(),
            'UnassignedNeighborhood' => $unassignedNeighborhood
        ]);
    }
     ...
     */
    public function SendRegretEmail(){
        $data = $this->_app->request->post();
        $requestSchema = new \Fortress\RequestSchema($this->_app->config('schema.path') . "/forms/sendmail.json");
        ...
        $template = $twig->loadTemplate("mail/RegretEmail.twig");
        ...
        $user->save();
        $ms->addMessageTranslated("success", "\REQUEST_SUCCESS");
    }
    ...
    public function register()
    {
        ...
        // connect users with unit
        $user->assignAppertment($user->id, $data['unit_id']);
    ...
    public function activate()
    {
		...
		Unit::where(["pending_user" => $user_id])
	                ->update(['pending_user'=> null,'user_id'=>$user_id]);
    ...
    public function setPassword($flag_new_user = false)
    {
        ...
        if (!$rf->validate()) {
            ...
            $this->_app->halt(400, "validate error");
        ...
        if (!$user) {
            ...
            $this->_app->halt(400, "user not found");
        ...
        if ($user->flag_password_reset == 0 || $last_password_reset_time === null) {
            $ms->addMessageTranslated("danger", "FORGOTPASS_INVALID_TOKEN");
            $this->_app->halt(400, "enter Check that a lost password request is in progress and has not expired");
        }
...
//            }
    ...
    public function sendmail()
    {
        ...
        // Load the user, by the specified email address
        $remaining = Transaction::leftjoin('uf_unit', 'uf_unit.id', '=',
            'uf_transactions.unit_id')->leftjoin('uf_user', 'uf_unit.user_id', '=',
            'uf_user.id')->leftjoin('uf_Payments', 'uf_payments.unit_id', '=', 'uf_unit.id')->
        where('uf_user.email', $data['email'])->select('uf_payments.remaining')->get();

        $duedate = Transaction::leftjoin('uf_unit', 'uf_unit.id', '=',
            'uf_transactions.unit_id')->leftjoin('uf_user', 'uf_unit.user_id', '=',
            'uf_user.id')->leftjoin('uf_Payments', 'uf_payments.unit_id', '=', 'uf_unit.id')->
        where('uf_user.email', $data['email'])->select('uf_payments.payment_due_date')->get();

        $amount = Transaction::leftjoin('uf_unit', 'uf_unit.id', '=',
            'uf_transactions.unit_id')->leftjoin('uf_user', 'uf_unit.user_id', '=',
            'uf_user.id')->leftjoin('uf_Payments', 'uf_payments.unit_id', '=', 'uf_unit.id')->
        where('uf_user.email', $data['email'])->select('uf_payments.payment_amount')->get();

        $total = Transaction::leftjoin('uf_unit', 'uf_unit.id', '=',
            'uf_transactions.unit_id')->leftjoin('uf_user', 'uf_unit.user_id', '=',
            'uf_user.id')->leftjoin('uf_Payments', 'uf_payments.unit_id', '=', 'uf_unit.id')->
        where('uf_user.email', $data['email'])->select('uf_payments.total_balance')->get();

        $user = User::where('email', $data['email'])->first();

        if (!$user) {
            $ms->addMessageTranslated("success", "sendmail_REQUEST_SUCCESS");
            $this->_app->halt(200);
        }


        $twig = $this->_app->view()->getEnvironment();
        $template = $twig->loadTemplate("mail/remainder-msg.twig");
        $notification = new Notification($template);
        $notification->fromWebsite();      // Automatically sets sender and reply-to
        $notification->addEmailRecipient($user->email, $user->display_name, [
            "user" => $user,
