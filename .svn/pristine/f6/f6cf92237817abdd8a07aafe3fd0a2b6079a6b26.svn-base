...
abstract class Database {
    ...
    public static function install(){
         ...
         * 'transactions' table.
         */
        if (!$schema->hasTable(static::getSchemaTable('transactions')->name)) {
            $schema->create(static::getSchemaTable('transactions')->name, function (Blueprint $table) {
                $table->increments('id');
                $table->dateTime('admin_transaction_date');
          ...
          /**
         * 'fixed_bills_operations' table.
         */
        if (!$schema->hasTable(static::getSchemaTable('fixed_bills_operations')->name)) {
            $schema->create(static::getSchemaTable('fixed_bills_operations')->name, function (Blueprint $table) {
                $table->increments('id');
                $table->integer('year');
                $table->integer('billAmount');
                $table->integer('extraFees');
                $table->integer('interestAmount');           
            });
        }
         ...
         * `neighborhood` table.
         */
        if (!$schema->hasTable(static::getSchemaTable('neighborhood')->name)) {
            $schema->create(static::getSchemaTable('neighborhood')->name, function (Blueprint $table) {
                $table->increments('id');
                $table->string('neighborhood_en', 255);
                $table->string('neighborhood_ar', 255);
                $table->engine = 'InnoDB';
                $table->collation = 'utf8_unicode_ci';
                $table->charset = 'utf8';  
            });
