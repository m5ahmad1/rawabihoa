...
class User extends UFModel
{
    ...
    public function checkAccess($hook, $params = [])
    {
        if ($this->isGuest()) {
            ...
            $this->logData("test isGuest False");
        ...
        if (empty($rule)) {
            $this->logData("test isGuest rule");
            $pass = false;
        } else {
            $this->logData("test isGuest rule else");
    ...
    {
        $res = User::leftjoin('uf_owner', 'uf_owner.user_id','=','uf_user.id')->select('uf_owner.name')->get();
 ...
 public function getUsersListt()
    {
         $transactions = Transaction::leftjoin('uf_unit', 'uf_unit.id', '=',
           ...
           ->select('uf_user.email','uf_user.display_name','uf_user.id','uf_payments.payment_due_date','uf_payments.payment_Amount','uf_payments.total_balance','uf_payments.remaining')->get();

        return $transactions;

    }
    //get all units for each user
    ...
    public function assignAppertment($uid, $unitsid)
    {
        if ($uid) {
            $arryOfUinitId = explode(",", $unitsid);

            for ($i = 0; $i < count($arryOfUinitId); $i++) {
                if ($arryOfUinitId[$i] > 0) {
                    $x = new Unit();
                    $x->assignUnitToUser($arryOfUinitId[$i], $uid);
                }
            }
        }
